...
// Tiny WebAudio toy-box: cozy synthesized SFX, no external files. Everything is
// guarded so missing/blocked audio silently disables sound without breaking the
// game. The context is created + resumed on the first user gesture.
const Sfx = (() => {
  let ctx=null, master=null, muted=false, noiseBuf=null, engine=null;
  function ensure(){ if(ctx) return; try{ const AC=window.AudioContext||window.webkitAudioContext; if(!AC) return; ctx=new AC(); master=ctx.createGain(); master.gain.value=.5; master.connect(ctx.destination); const n=Math.floor(ctx.sampleRate*.6); noiseBuf=ctx.createBuffer(1,n,ctx.sampleRate); const d=noiseBuf.getChannelData(0); for(let i=0;i<n;i++) d[i]=Math.random()*2-1; }catch(e){ ctx=null; } }
  function resume(){ ensure(); try{ if(ctx&&ctx.state==="suspended") ctx.resume(); }catch(e){} }
  function blip(freq=440,dur=.08,type="triangle",vol=.25){ if(muted) return; ensure(); if(!ctx) return; try{ const o=ctx.createOscillator(),g=ctx.createGain(),t=ctx.currentTime; o.type=type; o.frequency.setValueAtTime(freq,t); g.gain.setValueAtTime(.0001,t); g.gain.exponentialRampToValueAtTime(vol,t+.006); g.gain.exponentialRampToValueAtTime(.0001,t+dur); o.connect(g).connect(master); o.start(t); o.stop(t+dur+.02); }catch(e){} }
  function noise(dur=.22,freq=1400,q=.8,vol=.25){ if(muted) return; ensure(); if(!ctx||!noiseBuf) return; try{ const s=ctx.createBufferSource(),f=ctx.createBiquadFilter(),g=ctx.createGain(),t=ctx.currentTime; s.buffer=noiseBuf; f.type="bandpass"; f.frequency.value=freq; f.Q.value=q; g.gain.setValueAtTime(vol,t); g.gain.exponentialRampToValueAtTime(.0001,t+dur); s.connect(f).connect(g).connect(master); s.start(t); s.stop(t+dur+.02); }catch(e){} }
  function thud(vol=.5){ if(muted) return; ensure(); if(!ctx) return; try{ const o=ctx.createOscillator(),g=ctx.createGain(),t=ctx.currentTime; o.type="sine"; o.frequency.setValueAtTime(150,t); o.frequency.exponentialRampToValueAtTime(45,t+.18); g.gain.setValueAtTime(vol,t); g.gain.exponentialRampToValueAtTime(.0001,t+.22); o.connect(g).connect(master); o.start(t); o.stop(t+.24); }catch(e){} noise(.16,900,.6,.18); }
  function chime(){ [523,659,784,1046].forEach((f,i)=>setTimeout(()=>blip(f,.24,"triangle",.22),i*90)); }
  function startEngine(){ ensure(); if(!ctx||engine) return; try{ const o=ctx.createOscillator(),o2=ctx.createOscillator(),f=ctx.createBiquadFilter(),g=ctx.createGain(),t=ctx.currentTime; o.type="sawtooth"; o2.type="sawtooth"; o.frequency.value=90; o2.frequency.value=91.4; f.type="lowpass"; f.frequency.value=560; g.gain.value=0; o.connect(f); o2.connect(f); f.connect(g); g.connect(master); o.start(t); o2.start(t); engine={o,o2,f,g}; g.gain.linearRampToValueAtTime(muted?0:.1,t+.3); }catch(e){} }
  function updateEngine(intensity,air){ if(!engine||!ctx) return; try{ const t=ctx.currentTime,base=86+intensity*66+air*120; engine.o.frequency.setTargetAtTime(base,t,.08); engine.o2.frequency.setTargetAtTime(base*1.012,t,.08); engine.f.frequency.setTargetAtTime(480+intensity*900+air*700,t,.1); engine.g.gain.setTargetAtTime(muted?0:.085+intensity*.05,t,.1); }catch(e){} }
  function stopEngine(){ if(!engine||!ctx) return; const e=engine; engine=null; try{ e.g.gain.setTargetAtTime(0,ctx.currentTime,.15); setTimeout(()=>{try{e.o.stop();e.o2.stop();}catch(_){}} ,450); }catch(_){} }
  function setMuted(m){ muted=m; try{ if(master&&ctx) master.gain.setTargetAtTime(m?0:.5,ctx.currentTime,.05); }catch(e){} }
  return { resume, blip, noise, thud, chime, startEngine, updateEngine, stopEngine, setMuted, isMuted:()=>muted, ready:()=>!!ctx, engineOn:()=>!!engine };
})();
const ui = {
  ...
  hint: $("#toolHint"), loading: $("#loadingState"), muteBtn: $("#muteButton"),
...
function selectTool(tool,hint){if(racing)return;Sfx.resume();Sfx.blip(tool==="undo"?300:660,.05,"triangle",.16);clearPreview();if(tool==="undo")return undo();activeTool=tool;ui.hint.textContent=hint;document.querySelectorAll(".tool").forEach(button=>button.classList.toggle("active",button.dataset.tool===tool));}
...
canvas.addEventListener("pointerdown",event=>{if(racing)return;pokeCamera();Sfx.resume();lastPointer={x:event.clientX,y:event.clientY};if(event.shiftKey||event.button===1||event.button===2){panning=true;canvas.setPointerCapture(event.pointerId);return;}const point=pointerToSand(event);if(!inside(point))return;canvas.setPointerCapture(event.pointerId);snapshot();clearPreview();if(SCULPT.has(activeTool)){sculpting=true;lastSculpt=point.clone();applySculpt(point.x,point.z,activeTool);Sfx.noise(.26,activeTool==="carve"?800:1300,.7,.16);return;}if(activeTool==="track"){path=[point];drawing=true;rebuildTrack();}else if(activeTool==="start"){if(startMarker)disposeObject(startMarker);startMarker=makeMarker("start",point);animatePlacement(startMarker,"start");}else if(activeTool==="finish"){if(finishMarker)disposeObject(finishMarker);finishMarker=makeMarker("finish",point);animatePlacement(finishMarker,"finish");}else if(activeTool==="dozer"){obstacles=obstacles.filter(o=>{if(o.position.distanceTo(point)<2){disposeObject(o);return false;}return true;});combRing(point);}else if(DIFFICULTY[activeTool]!==undefined){const obstacle=makeObstacle(activeTool,point);obstacles.push(obstacle);animatePlacement(obstacle,activeTool);combRing(point);}if(activeTool==="start"||activeTool==="finish")combRing(point);if(activeTool==="dozer")Sfx.noise(.2,500,.5,.2);else if(activeTool!=="track")Sfx.blip(240,.11,"sine",.28);});
canvas.addEventListener("pointermove",event=>{pokeCamera();if(panning&&lastPointer){cameraTarget.x-=(event.clientX-lastPointer.x)*.025;cameraTarget.z-=(event.clientY-lastPointer.y)*.025;lastPointer={x:event.clientX,y:event.clientY};return;}if(racing)return;const point=pointerToSand(event);updateBrushRing(point);if(drawing){if(inside(point)&&(!path.length||path.at(-1).distanceTo(point)>.28)){const prev=path.at(-1);path.push(point);rebuildTrack();if(prev){const dx=point.x-prev.x,dz=point.z-prev.z,a=Math.atan2(-dz,dx),pl=Math.hypot(dx,dz)||1,px=-dz/pl,pz=dx/pl;for(const s of [-1,1]){const off=.95+Math.random()*.55;addComb(point.x+px*off*s,point.z+pz*off*s,a);}if(Math.random()<.4)addComb(point.x+px*(1.8+Math.random())*(Math.random()<.5?1:-1),point.z+pz*(1.8+Math.random()),a,true);}}return;}if(sculpting){if(inside(point)&&(!lastSculpt||lastSculpt.distanceToSquared(point)>.05)){applySculpt(point.x,point.z,activeTool);lastSculpt=point.clone();if(Math.random()<.5)Sfx.noise(.13,activeTool==="carve"?700:1500,.8,.07);}return;}updatePlacementPreview(point);});
...
function resetSandbox(){if(racing)return;clearPreview();snapshot();clearBuildObjects();path=[];riders.forEach(r=>disposeObject(r.group));riders=[];while(wearGroup.children.length)disposeObject(wearGroup.children[0]);clearCombs();flattenHeights();Sfx.noise(.4,700,.5,.22);ui.again.hidden=true;showFeedback("Fresh sand. What should we build this time?");}
...
function toggleMute(){Sfx.resume();const m=!Sfx.isMuted();Sfx.setMuted(m);ui.muteBtn.textContent=m?"🔇":"🔊";ui.muteBtn.setAttribute("aria-pressed",m?"true":"false");ui.muteBtn.title=m?"Unmute sound":"Mute sound";}
ui.muteBtn.onclick=toggleMute;
ui.race.onclick=startRace;ui.again.onclick=startRace;ui.reset.onclick=resetSandbox;window.addEventListener("resize",resize);window.addEventListener("keydown",event=>{if(event.code==="Space"){event.preventDefault();startRace();}if(event.key.toLowerCase()==="z"&&!racing)undo();if(event.key.toLowerCase()==="p"){event.preventDefault();setPhotoMode(!photoMode);}if(event.key.toLowerCase()==="m")toggleMute();if(event.key==="Escape"&&photoMode)setPhotoMode(false);});
window.__sandboxMotoDebug={camera:()=>({position:camera.position.toArray(),target:[camView.tx,camView.tz],idleMs:performance.now()-cameraIdleSince}),photo:()=>({on:photoMode,url:savePhoto(false)}),audio:()=>({ready:Sfx.ready(),muted:Sfx.isMuted(),engine:Sfx.engineOn()}),terrain:()=>{let mx=-1e9,mn=1e9;for(let i=0;i<hpos.count;i++){const y=hpos.getY(i);if(y>mx)mx=y;if(y<mn)mn=y;}return{max:mx,min:mn,verts:hpos.count};},placementState:()=>({preview:previewObject&&{type:previewObject.userData.type,snapped:previewObject.userData.snapped,opacity:previewObject.children[0]?.material?.opacity},building:buildAnimations.length,start:startMarker&&{position:startMarker.position.toArray(),rotation:startMarker.rotation.y,snapped:startMarker.userData.snapped},finish:finishMarker&&{position:finishMarker.position.toArray(),rotation:finishMarker.rotation.y,snapped:finishMarker.userData.snapped},obstacles:obstacles.map(object=>({type:object.userData.type,position:object.position.toArray(),rotation:object.rotation.y,snapped:object.userData.snapped}))})};
